  ...
  <div class="comment-item">
    ...
    <div class="comment-info">
      <div class="nick-name">
        ...
        <span class="user-status iconfont icon-user-filling">{{
          userStatus
        }}</span>
...
<script setup>
import { defineEmits, ref, getCurrentInstance, onMounted } from "vue";
...
const api = {
  ...
  getUserStatus: "/board/getUserStatus2",
};
...
const getUserStatus = async (userId, articleId) => {
...
const setUserStatus = async () => {
  userStatus.value = await getUserStatus(props.commentData.userId, props.articleId);
  if (props.commentData.children) {
    for (let sub of props.commentData.children) {
      sub.userStatus = await getUserStatus(sub.userId, props.articleId);
    }
  }
};
